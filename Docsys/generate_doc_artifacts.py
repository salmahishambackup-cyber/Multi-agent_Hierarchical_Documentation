...
def generate_doc_artifacts(cfg) -> Dict[str, Any]:
    """
    Stage A:
    ...
    writer = WriterAgent(hf=hf, google_prompt_path=Path("Agents/Prompts/google_docstring.md"))
    critic = CriticAgent(hf=hf, critic_prompt_path=Path("Agents/Prompts/critic_rules.md"))
    ...
    for module_path in plan.module_order:
        ...
        module_doc = writer.generate_docstring(prompt=module_prompt, llm_params=cfg.llm_params)
...
def _gen_symbol(cfg, extractor, writer, critic, module_path: str, node: Dict[str, Any], context_lines, dep_cache):
    ...
    doc = writer.generate_docstring(prompt=prompt, llm_params=cfg.llm_params)
    doc2, issues = critic.review(signature=signature, code_slice=code_slice, docstring=doc, llm_params=cfg.llm_params)
